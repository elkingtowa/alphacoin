...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1000000000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1000000000); // Litecoin: 1000000000 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Alphacoin: 1 days
static const int64 nTargetSpacing = 0.1 * 60; // Alphacoin: .1 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Harvard Crimson 04/Feb/2014 Alphacoin is born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
